...
function generateTimeSeriesData(baseValue: number, volatility: number): TimeSeriesDataPoint[] {
...
const salesCategoriesData: PieChartItem[] = [
...
const deliveryData: PieChartItem[] = [
...
function createLineChart(elementId: string, data: TimeSeriesDataPoint[], color: string, fillArea: boolean): void {
...
function createPieChart(elementId: string, data: PieChartItem[]): void {
  const ctx = document.getElementById(elementId) as HTMLCanvasElement;
  if (!ctx) return;
