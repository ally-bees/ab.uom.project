...
export class TaxTableService {
  ...
  getTaxRecords(): Observable<TaxRecord[]> {   //Observable<TaxRecord[]> is return type. method returns an Observable that emits an array of TaxRecord objects.
    // Return mock data as an Observable (simulating HTTP request)
    return of(this.taxRecords); //of() is an RxJS function that creates an Observable from the given data.this.taxRecords stores inside the service
  }
}